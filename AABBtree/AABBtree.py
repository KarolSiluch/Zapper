...
from base_tile.modules.Position2DModule import RectType
from typing import Protocol
from base_tile.modules.basic_modules import Context
import camera.game_camera as camera
from base_tile.modules.basic_modules import ModuleType
from base_tile.modules.renderer import BaseRenderer
...
class YSortCAmera(AABBTree):
    def render(self, surface: pygame.Surface, camera: camera.Camera):
        tiles: tuple[Context] = self.RectCollision(camera.rect)
        camera_offset = camera.offset

        renderets: list[BaseRenderer] = [tile.get_module(ModuleType.Renderer) for tile in tiles]

        for renderer in sorted(renderets, key=lambda renderer: renderer.top()):
            renderer.render(surface, -camera_offset + renderer.topleft())
